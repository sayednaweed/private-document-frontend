...
export default function SourceDialog(props: SourceDialogProps) {
  const { onComplete, source } = props;
  ...
  const fetch = async () => {
    try {
      const response = await axiosClient.get(`source/${source?.id}`);
      if (response.status === 200) {
        setUserData({
          Farsi: response.data.source.fa,
          English: response.data.source.en,
          Pashto: response.data.source.ps,
        });
      }
    } catch (error: any) {
      console.log(error);
  ...
  const store = async () => {
    try {
      ...
      const response = await axiosClient.post("source/store", formData);
      if (response.status === 200) {
        toast({
          toastType: "SUCCESS",
          description: response.data.message,
        });
        onComplete(response.data.source);
        modelOnRequestHide();
      }
    } catch (error: any) {
  ...
  const update = async () => {
    try {
      ...
      // 2. update
      let formData = new FormData();
      if (source?.id) formData.append("id", source.id);
      formData.append("english", userData.English);
      formData.append("farsi", userData.Farsi);
      formData.append("pashto", userData.Pashto);
      const response = await axiosClient.post(`source/update`, formData);
      if (response.status === 200) {
        toast({
          toastType: "SUCCESS",
          description: response.data.message,
